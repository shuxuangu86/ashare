...
pid_file="artifacts/logs/published-formula-evaluation.pid"
log_file="artifacts/logs/published-formula-evaluation.log"
...
if [[ "${1:-}" == "--worker" ]]; then
  run_pack alpha101_original_v1 l2-v2-alpha101-5y-v1
  run_pack alpha191_original_v1 l2-v2-alpha191-5y-v1
  exit 0
fi

nohup bash scripts/launch_published_formula_evaluation.sh --worker \
  >"${log_file}" 2>&1 &
echo "$!" >"${pid_file}"
