...
按用户指示，L3 v1 仅使用市值中性输入，暂不等待行业数据。家族方法只使用前两折
的均值、稳定性和复杂度选择，第三折不参与选择。19 个家族的第三折 RankIC 全部为
正，平均为 0.0399；最终冻结 9 个等权、8 个滚动 IC、1 个滚动 ICIR 和 1 个 Ridge
家族模型。它们将作为下一阶段跨家族 walk-forward 的输入。
